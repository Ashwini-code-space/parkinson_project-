"""
parkinson_ml_pipeline_modified.py
Modified ML pipeline for Parkinson dataset:
- Loads fixed training files (PD CSV + Healthy XLSX), combines, preprocesses, trains models.
- Prompts for test file (CSV or XLSX), assumes no 'Label' column, predicts per model per sample.
- Outputs predictions CSV with 'Sample' IDs and per-model predictions (HC/PwPD).
- Computes cross-validation metrics on training data (accuracy, precision, recall, F1, ROC AUC).
- Saves models, CV metrics, feature importance plots, and learning curve to ml_results/
- Adds recommended ensembles: Stacking (SVM, RF, XGB), GB+RF, Voting (SVM, KNN, GB), ANOVA+Stacking, Bagging+Boosting.
"""
import os
import warnings
...
# Config / paths
# ---------------------------
TRAIN_PD_CSV =  " training dataset for pd.csv"  # Fixed training PD file
TRAIN_HEALTHY_XLSX = "training dataset healthy.csv"  # Fixed training healthy file
...
df_train_healthy = pd.read_excel(TRAIN_HEALTHY_XLSX, sheet_name="Sheet1")
# Combine
